...
// 			specular: 	0x333333,
	...
	var vizilines = buildDataVizGeometries(conflictReport);
	...
	selectVisualization(conflictReport);	
        
        setInterval(function() {
            loadContentData(function() {
                buildDataVizGeometries( conflictReport );
                selectVisualization(conflictReport);
            });
        }, 1000*20);
...
function highlightCountry( countries ){	
	...
	for( var i in countryCodes ){
		...
		if (countryData[countries[i]] == undefined) {
			console.warn('Undefined country data.', countries[i]);
			continue;
		}
